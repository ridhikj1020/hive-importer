...
import { renameSection } from '@/lib/db/queries';
...
export async function PATCH(req: Request, ctx: IdContext) {
  return guard(async () => {
    ...
    const ok = await renameSection(getDb(), id, name);
    return ok ? Response.json({ id, name }) : fail(404, 'NOT_FOUND', 'That section does not exist.');
  });
}
